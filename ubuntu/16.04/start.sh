...
VSTS_AGENT_RESPONSE=$(curl -LsS \
  ...
  -H 'Accept:application/json;api-version=4.1' \
  "https://$VSTS_ACCOUNT.visualstudio.com/_apis/distributedtask/packages/agent/linux-x64")

echo $VSTS_AGENT_RESPONSE
...
if [ -z "$VSTS_AGENT_URL" -o "$VSTS_AGENT_URL" == "null" ]; then
  echo 1>&2 big error: could not determine a matching VSTS agent - check that account \'$VSTS_ACCOUNT\' is correct and the token is valid for that account
  exit 1
fi
